        ...
        <form method="post" action="/aetherupload">
            <div class="form-group " id="aetherupload-wrapper"><!--组件最外部需要有一个名为aetherupload-wrapper的id，用以包装组件-->
                ...
                <div class="controls">
                    ...
                    <!--需要有一个名为resource的id，用以标识上传的文件，aetherupload(...)中第二个参数为分组名，success方法可用于声名上传成功后的回调方法名-->
            ...
            <div class="form-group " id="aetherupload-wrapper">
                <label>文件2(无回调)：</label>
                <div class="controls">
                    <input type="file" id="resource" onchange="aetherupload(this,'file').upload()"/>
                    <div class="progress " style="height: 6px;margin-bottom: 2px;margin-top: 10px;width: 200px;">
                        <div id="progressbar" style="background:blue;height:6px;width:0;"></div>
                    </div>
            ...
            <input type="hidden" id="aetherupload-storage-host" value="{{config('aetherupload.DISTRIBUTED_DEPLOYMENT.ENABLE')&&(config('aetherupload.DISTRIBUTED_DEPLOYMENT.ROLE') === 'web')?config('aetherupload.DISTRIBUTED_DEPLOYMENT.WEB.STORAGE_HOST'):''}}" /><!--需要有一个名为aetherupload-storage-host的id，用以标识储存服务器的host-->
        {{ csrf_field() }} <!--需要有csrf token-->
...
<script>
    ...
    someCallback = function () {
        // Example
        $('#result').append(
            '<p>执行回调 - 文件原名：<span >' + this.resourceName + '</span> | 文件大小：<span >' + parseFloat(this.resourceSize / (1000 * 1000)).toFixed(2) + 'MB' + '</span> | 文件储存名：<span >' + this.savedPath.substr(this.savedPath.lastIndexOf('_') + 1) + '</span></p>'
        );
    }
